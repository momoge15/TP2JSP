...
@WebServlet(name = "ServletUsers",
        ...
        initParams = {
            @WebInitParam(name = "ressourceDir", value = "C:\\Users\\Wili\\Documents\\NetBeansProjects\\jsp-master")
        }
)
public class ServletUsers extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action != null) {
            if (action.equals("listerLesUtilisateurs")) {
                ...
                User user = supprimerUtilisateur(request);
                Collection<User> liste = Server.uh.getUsers();
               // ArrayList<User> liste = new ArrayList<User>();
                liste.remove(user);
                request.setAttribute("listeDesUsers", liste);
    ...
    private void creeUtilisateurDeTest() {
        ...
        try {
            User user1 = new User("AMG", "Assi", "Maurel");
            User user2 = new User("MSH", "Madou", "Silvie");
            User user3 = new User("MG", "Maelle", "Gaelle");
    ...
    public User rechercherUtilisateur(HttpServletRequest request) {
     ...
     public void modifierUtilisateur(HttpServletRequest request) {
        ...
        Server.uh.updateUser(login, login, prenom, nom);

    }
     
     public User supprimerUtilisateur(HttpServletRequest request) {
        String login;
        login = request.getParameter("login");
